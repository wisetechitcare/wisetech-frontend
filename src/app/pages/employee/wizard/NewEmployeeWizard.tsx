...
function NewEmployeeWizard({ editMode, openModal }: any) {
  ...
  const updateWizardData = async (values: any) => {
    ...
    Object.keys(employeePayload).forEach((key) => {
      // `avatar` is exempt: an empty string is the intended "photo removed" value and
      // must reach the backend (which clears it) — otherwise the strip below drops it
      // and the old picture survives the save.
      if (key === "gender" || key === "maritalStatus" || key === "isHiddenFromStaff" || key === "avatar" || PROF_FEES_KEYS.has(key)) return;
