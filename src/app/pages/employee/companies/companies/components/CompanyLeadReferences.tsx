...
const CompanyLeadReferences: React.FC<{ referredLeads?: LeadReferral[] }> = ({ referredLeads = [] }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      Cell: ({ row }: any) => {
        const ref: LeadReferral = row.original;
        if (ref.referredByContact) return <span>{ref.referredByContact.fullName}</span>;
        if (ref.referredByEmployee?.users) {
          const { firstName, lastName } = ref.referredByEmployee.users;
