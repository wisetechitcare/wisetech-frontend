...
const ContactConfigMain = () => {
  ...
  const [showCompanyRoleTypeModal, setShowCompanyRoleTypeModal] =
    useState(false);
  const [editingCompanyRoleType, setEditingCompanyRoleType] = useState<
    any | null
  >(null);
  const [showContactStatusModal, setShowContactStatusModal] = useState(false);
  const [editingContactStatus, setEditingContactStatus] = useState<any | null>(
    null,
  );
    ...
    <div>
      ...
      >
        <div className="card-body">
          ...
          <div className="row mt-4">
            {[...contactRoleTypes].sort((a: any, b: any) => (a.name || "").localeCompare(b.name || "")).map((contactRoleType: any) => (
              <div key={contactRoleType.id} className="col-12 col-md-3 mb-3">
                ...
                >
                  <div className="d-flex align-items-center gap-2">
                    ...
                    >
                      {contactRoleType.name.length > 10
                        ? contactRoleType.name.slice(0, 10) + "..."
                        : contactRoleType.name}
                    </div>
                  </div>
                  <div className="ms-4 d-flex gap-3">
                    ...
                    <i
                      ...
                      onClick={() =>
                        handleContactRoleTypeDelete(contactRoleType.id)
                      }
                    ></i>
                  </div>
                </div>
      ...
      >
        <div className="card-body">
          ...
          <div className="row mt-4">
            {[...contactStatuses].sort((a: any, b: any) => (a.name || "").localeCompare(b.name || "")).map((contactStatus: any) => (
              <div key={contactStatus.id} className="col-12 col-md-3 mb-3">
                ...
                >
                  <div className="d-flex align-items-center gap-2">
                    ...
                    >
                      {contactStatus.name.length > 10
                        ? contactStatus.name.slice(0, 10) + "..."
                        : contactStatus.name}
                    </div>
                  </div>
                  <div className="ms-4 d-flex gap-3">
                    ...
                    <i
                      className="fa fa-trash cursor-pointer"
                      onClick={() =>
                        handleContactStatusDelete(contactStatus.id)
                      }
                    ></i>
                  </div>
                </div>
