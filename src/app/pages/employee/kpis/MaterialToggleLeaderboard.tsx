...
import Daily from '@pages/employee/kpis/common/Daily';
...
import AllTime from './common/AllTime';
...
const MaterialToggleLeaderboard = ({
    toggleItemsActions,
    fromAdmin = false,
    resourseAndView,
    dateSettingsEnabled
...
const [weekEnd, setWeekEnd] = useState(() => {
  ...
  if (dateSettingsEnabled) {
    ...
    const [monthEnd, setMonthEnd] = useState(dateSettingsEnabled ? today : today.endOf('month'));
    ...
    useEffect(() => {
        ...
        async function calculateFiscalYear() {
            ...
            const fiscalEnd = isCurrentFiscalYear(fiscalStart, dayjs(endDate)) && dateSettingsEnabled
                ? today
                : dayjs(endDate);

            setYearStart(fiscalStart);
            setYearEnd(fiscalEnd);
            setFiscalYearDisplay(`${fiscalStart.format('DD MMM, YYYY')} - ${fiscalEnd.format('DD MMM, YYYY')}`);
    ...
    const navigateDay = (direction: 'prev' | 'next') => {
        const newDay = direction === 'prev' ? day.subtract(1, 'day') : day.add(1, 'day');
        setDay(newDay);
    ...
    const navigateMonth = (direction: 'prev' | 'next') => {
        ...
        const newMonthEnd = dateSettingsEnabled && newMonthStart.isSame(today, 'month')
            ? today
    ...
    return (
        <>
            <div className="d-flex flex-md-row flex-column justify-content-lg-between align-items-lg-center justify-content-start align-items-start">
                <div className="col-12 col-sm-auto mb-6" >
                    <ToggleButtonGroup
                        value={alignment}
                        exclusive
                ...
                {alignment === 'weekly' && (
                    <NavigationButtons
                        ...
                        displayText={`${weekStart.format('DD MMM, YYYY')} - ${weekEnd.format('DD MMM, YYYY')}`}
                    />
                )}
                ...
                {alignment === 'monthly' && (
                    <NavigationButtons
                        ...
                        displayText={`${monthStart.format('DD MMM, YYYY')} - ${monthEnd.format('DD MMM, YYYY')}`}
                    />
                )}
            ...
            {/* {alignment === 'daily' && <Daily day={day} fromAdmin={fromAdmin} resourseAndView={resourseAndView} />} */}
            {alignment === 'weekly' && (
                <Weekly
                    startWeek={weekStart}
                    endWeek={weekEnd}
...
export default MaterialToggleLeaderboard;
