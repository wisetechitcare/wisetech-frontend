...
const Custom = ({ startDate, endDate }: Props) => {
  ...
  const handleServiceChartClick = (selectedLabel: string) => {
    console.log("Service filter changed to:", selectedLabel);
  ...
  const handleCompanyTypeChartClick = (selectedLabel: string) => {
    ...
    // console.log("companyTypeRes",companyTypeRes);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const startDates = dayjs(startDate).format("YYYY-MM-DD");
        const endDates = dayjs(endDate).format("YYYY-MM-DD");
        setError("");
  ...
  // console.log("filters.subcategoryCategory", filters.subcategoryCategory);
  ...
  return (
    <div className="container-fluid">
      <div className="row g-4">
        ...
        {settings?.showLeadsByProjectCategory && (
            <div className="col-12 col-md-6">
              <CustomPieCharts
                ...
                // Add loading state if your CustomPieCharts component supports it
                // loading={loading}
              />
            </div>
          )}
        ...
        {settings?.showLeadsFromReferral && (
            <div className="col-12 col-md-6">
              <CustomPieCharts
                ...
                title={
                  filters.referralStatus && filters.referralStatus !== ""
                    ? `Referral Sources - ${filters.referralStatus}`
                    : "Lead By Referral Source"
                }
                height={250}
                chartType="donut"
        ...
        {settings?.showLeadsByYearlyDirectSource && (
            <YearlyStatusCountChart
