...
const YearlyCumulativePerformance: React.FC<YearlyCumulativePerformanceProps> = ({ 
  ...
  const [targetTypeToEdit, setTargetTypeToEdit] = useState<"inquiry" | "received" | null>(null);
  const [rawData, setRawData] = useState<any>(null);
  const [monthlyTargets, setMonthlyTargets] = useState<{ inquiry: any[], received: any[] }>({
    inquiry: [],
    received: []
  });
  ...
  const transformedData = useMemo(() => {
    ...
    const activeTargets = viewMode === "inquiry" ? monthlyTargets.inquiry : monthlyTargets.received;
    ...
    return rawData.data.map((item: any, index: number) => {
      const monthlyActual = viewMode === "inquiry" ? item.inquiry : item.received;
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      const actual = payload.find((p: any) => p.dataKey === "cumulativeValue")?.value || 0;
      const targetPath = payload.find((p: any) => p.dataKey === "target")?.value || 0;
      const runRate = payload.find((p: any) => p.dataKey === "runRate")?.value || 0;
      const gap = Number((actual - runRate).toFixed(2));

      return (
        <div className="bg-white p-3 shadow-lg rounded-3 border">
          <p className="fw-bold text-dark mb-2 border-bottom pb-1">{label} {currentDate.format('YYYY')} - {viewMode.toUpperCase()}</p>
          
          <div className="d-flex justify-content-between gap-4 mb-1">
            <span className="text-primary font-weight-bold" style={{ fontSize: '13px' }}>Actual (Cumul.):</span>
            <span className="fw-bold" style={{ fontSize: '13px' }}>{actual}</span>
          </div>
          
          <div className="d-flex justify-content-between gap-4 mb-1">
            <span className="text-secondary font-weight-bold" style={{ fontSize: '13px' }}>Cumulative Target:</span>
            <span className="fw-bold" style={{ fontSize: '13px' }}>{targetPath}</span>
          </div>

          <div className="d-flex justify-content-between gap-4 mb-1">
            <span className="text-info font-weight-bold" style={{ fontSize: '13px' }}>Reference Pace:</span>
            <span className="fw-bold" style={{ fontSize: '13px' }}>{runRate}</span>
          </div>

          <div className="mt-2 pt-2 border-top d-flex justify-content-between gap-4">
            <span className={gap >= 0 ? "text-success fw-bold" : "text-danger fw-bold"} style={{ fontSize: '13px' }}>
              Pace Variance:
            </span>
            <span className={gap >= 0 ? "text-success fw-bold" : "text-danger fw-bold"} style={{ fontSize: '13px' }}>
              {gap > 0 ? `+${gap}` : gap}
  ...
  const finalYearlyTarget = transformedData[transformedData.length - 1]?.totalYearlyTarget || 0;

  return (
    <div className={`${isExpanded ? "fixed-top w-100 h-100 p-4 bg-white" : "w-100"}`} style={{ zIndex: 1070 }}>
      <Card className={`border-0 rounded-4 overflow-hidden shadow-sm ${isExpanded ? "h-100" : "mb-4"}`} style={{ background: '#FFFFFF', border: '1px solid #F1F5F9' }}>
        <div className="p-4 border-bottom d-flex justify-content-between align-items-center flex-wrap gap-3">
          <div className="d-flex flex-column gap-1">
              <h5 className="fw-bold m-0 text-dark" style={{ letterSpacing: '-0.01em' }}>{title}</h5>
              <div className="d-flex align-items-center gap-2">
                <div className="d-flex align-items-center bg-light rounded-3 p-1 border shadow-sm">
                  ...
                  >
                    <i className="bi bi-chevron-left" style={{ fontSize: '14px', fontWeight: '800' }}></i>
                  </Button>
                  <div className="px-3 border-start border-end">
                    <span className="fw-bold text-primary m-0" style={{ fontSize: '13px', minWidth: '80px', textAlign: 'center', display: 'inline-block' }}>
                  ...
                  <Button 
                    ...
                    style={{ textDecoration: 'none' }}
                  >
                    <i className="bi bi-chevron-right" style={{ fontSize: '14px', fontWeight: '800' }}></i>
                  </Button>
                </div>
                <p className="text-muted m-0" style={{ fontSize: '11px', opacity: 0.8 }}>Dynamic Yearly Performance Analytics</p>
              </div>
          </div>
          ...
          <div className="d-flex align-items-center gap-3">
            {viewMode === "inquiry" ? (
              <Button 
                  ...
                  style={{ fontSize: '11px', backgroundColor: '#EEF2FF', color: '#4F46E5', borderRadius: '8px' }}
              >
              ...
              <Button 
                  ...
                  style={{ fontSize: '11px', backgroundColor: '#ECFDF5', color: '#059669', borderRadius: '8px' }}
              >
                  <i className="bi bi-gear-fill"></i>
                  SET RECEIVED TARGET
              </Button>
            ...
            <Button variant="light" size="sm" className="border shadow-sm rounded-3 p-2 bg-white" onClick={toggleExpand}>
              <i className={`bi ${isExpanded ? 'bi-fullscreen-exit' : 'bi-arrows-fullscreen'}`} style={{ fontSize: '14px' }}></i>
            </Button>
          </div>
        </div>

        <Card.Body className="p-4">
          <div style={{ width: '100%', height: isExpanded ? 'calc(100% - 140px)' : '380px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={transformedData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                ...
                <Line 
                    ...
                    strokeWidth={3} 
                    dot={{ r: 4, fill: viewMode === 'inquiry' ? "#2563EB" : "#10B981" }}
                    activeDot={{ r: 6 }}
        ...
        <div className="px-4 pb-4">
          <div className="bg-light rounded p-3 d-flex justify-content-between align-items-center">
            <div className="text-muted small">
              <span className="fw-bold text-dark">{viewMode === 'received' ? 'Yearly Goal: ' : 'Yearly Goal: '}</span>
              {finalYearlyTarget}
            </div>
            <div className="text-muted small">
              <span className="fw-bold text-dark">Current Actual: </span>
              {transformedData[transformedData.length - 1]?.cumulativeValue || 0}
            </div>
            <div className="text-muted small">
              <span className="fw-bold text-dark">Success Rate: </span>
              <span className={viewMode === 'received' ? 'text-success fw-bold' : 'text-primary fw-bold'}>
                {finalYearlyTarget > 0 ? Math.round((transformedData[transformedData.length - 1]?.cumulativeValue / finalYearlyTarget) * 100) : 0}%
              </span>
            </div>
          </div>
        </div>
