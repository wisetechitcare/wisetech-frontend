...
import { generateRevisionFileName, getExportTypeConfig } from '../utils/dmsUtils';
import type { ExportType, ExportDestination, ExportProgress, ExportStep } from '../types/dms.types';
...
export const ExportCenterModal: React.FC<ExportCenterModalProps> = ({
  ...
  const handleStartExport = async (force: boolean = false) => {
    ...
    const steps = destination === 'device' 
      ? allSteps.filter(s => !['4', '5'].includes(s.id))
      : allSteps;

    setProgress({ steps, uploadProgress: 0, exportProgress: 0, isComplete: false, hasError: false });
    ...
    try {
      ...
      setProgress(prev => prev ? { ...prev, hasError: true, errorMessage: error.message } : null);
      setIsExporting(false);
    }
  };
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{
          background: 'white', borderRadius: '32px', width: '680px', maxWidth: '100%',
          ...
          {showConfirmReplace && (
            ...
            >
              <div style={{ maxWidth: '420px' }}>
                ...
                <h3 style={{ fontSize: '24px', fontWeight: 800, color: '#1e293b', marginBottom: '16px', fontFamily: 'Barlow' }}>
                  Revision {String(currentLeadRev).padStart(2, '0')} Exists
                </h3>
                <p style={{ color: '#64748b', marginBottom: '40px', lineHeight: 1.6, fontSize: '15px' }}>
                  An official {exportType} for this revision already exists. 
                  Continuing will <strong style={{ color: '#ef4444' }}>overwrite</strong> the existing file in the DMS Global Repository.
                </p>
                <div style={{ display: 'flex', gap: '16px' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: '#9d4141', color: 'white', fontWeight: 700, cursor: 'pointer',
                      boxShadow: '0 10px 25px rgba(157,65,65,0.25)', transition: 'all 0.2s'
                    }}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <div style={{ 
              ...
              background: 'rgba(157, 65, 65, 0.08)', display: 'flex', 
              alignItems: 'center', justifyContent: 'center' 
            }}>
              <KTIcon iconName="send" className="fs-2 text-primary" />
            </div>
            <div>
              <h2 style={{ margin: 0, fontSize: '22px', fontWeight: 800, color: '#1e293b', fontFamily: 'Barlow' }}>Export Center</h2>
              <p style={{ margin: '2px 0 0', fontSize: '13px', color: '#64748b', fontWeight: 500 }}>Lead-Controlled Revision Workflow</p>
            </div>
          </div>
          <button 
            onClick={onHide} 
            ...
            <div style={{ maxWidth: '440px', margin: '0 auto' }}>
              <div style={{ marginBottom: '40px', textAlign: 'center' }}>
                <h3 style={{ 
                  ...
                  marginBottom: '12px', fontFamily: 'Barlow' 
                }}>
                  {progress?.isComplete ? 'Export Complete' : 'Processing Document'}
                </h3>
                <p style={{ fontSize: '14px', color: '#64748b', fontFamily: 'Inter' }}>
                  Preparing your files for the DMS Repository. This may take a few moments.
                </p>
              </div>
              ...
              <div style={{ 
                ...
                borderRadius: '10px', marginBottom: '40px', overflow: 'hidden',
                position: 'relative'
              }}>
                <motion.div
                  initial={{ width: '0%' }}
                  animate={{ 
                    width: progress?.isComplete ? '100%' : 
                           `${((progress?.steps.filter(s => s.status === 'success').length || 0) / (progress?.steps.length || 5)) * 100}%` 
                  }}
                  transition={{ duration: 0.5 }}
                  style={{ height: '100%', background: '#9d4141', borderRadius: '10px' }}
                />
              </div>
                ...
                {progress?.steps.map((step, idx) => (
                  <div key={step.id} style={{ 
                    ...
                    <div style={{
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      background: step.status === 'success' ? '#9d4141' : step.status === 'running' ? 'rgba(157,65,65,0.1)' : '#f8fafc',
                      border: step.status === 'running' ? '1px solid rgba(157,65,65,0.2)' : '1px solid #f1f5f9',
                      transition: 'all 0.3s'
                    }}>
                      {step.status === 'success' ? (
                        <KTIcon iconName="check" className="fs-5 text-white" />
                      ) : step.status === 'running' ? (
                        <motion.div
                          animate={{ rotate: 360 }}
                          transition={{ duration: 1.5, repeat: Infinity, ease: 'linear' }}
                          style={{ width: '12px', height: '12px', border: '2px solid #9d4141', borderTopColor: 'transparent', borderRadius: '50%' }}
                        />
                      ) : (
                    ...
                    <span style={{
                      fontSize: '14px', fontWeight: step.status === 'running' ? 700 : 500,
                      color: step.status === 'pending' ? '#94a3b8' : '#334155',
                      fontFamily: 'Inter'
                    }}>
                      {step.label}
                    </span>
                    {step.status === 'running' && (
                      <motion.span
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        style={{ fontSize: '12px', color: '#9d4141', fontWeight: 600, marginLeft: 'auto' }}
                      >
                        In Progress...
                      </motion.span>
                    )}
              ...
              {progress?.isComplete && (
                <motion.div
                  ...
                  style={{ 
                    marginTop: '40px', padding: '16px', background: '#fef2f2', 
                    borderRadius: '12px', border: '1px solid #fee2e2',
                    color: '#9d4141', fontWeight: 600, fontSize: '14px',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px'
                  }}
                >
                  <KTIcon iconName="check-circle" className="fs-3 text-primary" />
                  File successfully synchronized with DMS
                </motion.div>
              )}
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          background: active ? 'rgba(157, 65, 65, 0.03)' : 'white',
                          cursor: 'pointer', transition: 'all 0.25s',
                          boxShadow: active ? '0 10px 25px -10px rgba(157, 65, 65, 0.15)' : 'none'
                        }}
                      >
                        <div style={{ 
                        ...
                        <div style={{ fontSize: '15px', fontWeight: 800, color: active ? '#1e293b' : '#475569', marginBottom: '4px' }}>{cfg.label}</div>
                        <div style={{ fontSize: '12px', color: '#94a3b8', lineHeight: 1.4 }}>{cfg.description}</div>
                      </div>
                    );
                  })}
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
                <div>
                  ...
                  <div style={{ 
                    display: 'flex', gap: '4px', marginTop: '12px', background: '#f1f5f9', 
                    padding: '4px', borderRadius: '12px' 
                  }}>
                    {(['device', 'cloud', 'both'] as ExportDestination[]).map(dest => (
                      <div
                        key={dest}
                        onClick={() => setDestination(dest)}
                        style={{
                          flex: 1, padding: '8px 4px', borderRadius: '8px', textAlign: 'center',
                          background: destination === dest ? 'white' : 'transparent',
                          color: destination === dest ? '#1e293b' : '#64748b',
                          boxShadow: destination === dest ? '0 4px 6px -1px rgba(0,0,0,0.1)' : 'none',
                ...
                <div>
                  ...
                  <div style={{ 
                    marginTop: '12px', background: '#f8fafc', padding: '10px 16px', 
                    ...
                    <div style={{ 
                      background: '#9d4141', color: 'white', padding: '2px 8px', 
              ...
              <div>
                ...
                <div style={{ position: 'relative', marginTop: '12px' }}>
                  <input
                    ...
                    style={{
                      width: '100%', padding: '16px 48px 16px 16px', borderRadius: '16px',
                  ...
                  <div style={{ position: 'absolute', right: '16px', top: '16px' }}>
                    <KTIcon iconName="document" className="fs-2 text-gray-400" />
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <KTIcon iconName="shield-tick" className="fs-6 text-gray-400" />
                    <p style={{ margin: 0, fontSize: '12px', color: '#94a3b8', fontWeight: 500 }}>
                      Controlled by Lead Revision status.
                    </p>
                  </div>
                  <button 
                    onClick={(e) => { e.preventDefault(); setTempNumber(prev => prev + 1); }}
                    style={{
                      background: 'none', border: 'none', padding: 0, color: '#9d4141',
              ...
              <div style={{ marginTop: '8px' }}>
                <motion.button
                  whileHover={{ y: -2, boxShadow: '0 15px 35px rgba(157, 65, 65, 0.3)' }}
                  whileTap={{ y: 0 }}
                  disabled={isExporting}
                  onClick={() => handleStartExport(false)}
                  style={{
                    width: '100%', padding: '18px', borderRadius: '20px',
                    background: 'linear-gradient(135deg, #9d4141 0%, #bd4b4b 100%)',
                    color: 'white', border: 'none', fontSize: '17px', fontWeight: 800,
