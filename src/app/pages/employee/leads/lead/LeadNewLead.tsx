            ...
            if (leadsData.length > 0) {
              ...
              const transformedLeads = leadsData.map((lead: any) => {
                ...
                return {
                  ...
                  // cost accessor now stores the RATE from commercials as requested
                  cost: Array.isArray(lead.commercials) && lead.commercials.length > 0 ? (parseFloat(lead.commercials[0]?.rate) || 0) : (lead.rate || 0),
                  ...
                  budget: Array.isArray(lead.commercials) && lead.commercials.length > 0
                    ? lead.commercials.reduce((sum: number, c: any) => sum + (parseFloat(c.cost) || 0), 0)
                    : (lead.budget || ""),
                  // rate accessor (optional, since 'cost' now shows rate)
                  rate: Array.isArray(lead.commercials) && lead.commercials.length > 0 ? lead.commercials[0]?.rate : (lead.rate || ""),
