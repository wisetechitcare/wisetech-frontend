...
/**
 * Fetch projects for a reimbursement form, filtered server-side to only those
 * whose status is "On Ongoing" (resolved dynamically from the project status master table).
 *
 * @param companyId        - The client company whose projects to load
 * @param includeProjectId - (edit mode only) A project id that must always be included
 *                           in the result even if its status has changed away from
 *                           "On Ongoing", so historical reimbursement records stay intact.
 */
export const getProjectsByCompanyIdForReimbursement = async (
    companyId: string,
    includeProjectId?: string
) => {
    try {
        const endpoint = `${API_BASE_URL}/${LEAD_PROJECT_COMPANY.GET_PROJECTS_BY_COMPANY_ID_FOR_REIMBURSEMENT.replace(":companyId", companyId)}`;
        const params: Record<string, string> = {};
        if (includeProjectId) {
            params.includeProjectId = includeProjectId;
        }
