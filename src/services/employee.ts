...
export const getPaymentHistory = async (salaryId: string) => {
    try {
        const endpoint = `${API_BASE_URL}/api/company/salary/payments/${salaryId}`;
        const { data } = await axios.get(endpoint);
        return data;
    } catch (err) {
        throw err;
...
export const fetchEmpKpiStatisticsForDay = async (
    employeeId: string,
    date: string
) => {
    return fetchEmpKpiStatisticsForPeriod(employeeId, date, date);
};
...
export const fetchAllStarEmployeeByStartAndEndDate = async (
  startDate?: string,
  endDate?: string
) => {
  try {
    const endpoint = `${API_BASE_URL}/api/employee/kpi/star-employees-all`;
    ...
    console.log("🔥 STAR EMPLOYEE API HIT:", endpoint);
    console.log("🔥 STAR EMPLOYEE RESPONSE:", data);

    return data;
  } catch (err) {
    console.error("❌ Star Employees API Error:", err);
    throw err;
  }
};
...
export const fetchStarEmployeesByFactorId = async (factorId: string) => {
    try {
        ...
        console.error("Error fetching star employees by factor:", err);
        throw err;
    }
}
