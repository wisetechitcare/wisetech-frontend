...
import { categoryMeta, initials } from './tokens';
import { fromNow } from './time';

const Kpi: React.FC<{ icon: string; label: string; value: React.ReactNode; accent: keyof typeof ICON_COLORS }> = ({
  icon,
  label,
  value,
  accent,
  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ fontFamily: FONT.heading, fontSize: 19, fontWeight: 700, color: C.textPrimary, lineHeight: 1.1 }}>
          {value}
        </div>
...
const Bar: React.FC<{ label: React.ReactNode; count: number; max: number; color: string }> = ({
  label,
  count,
  max,
  color,
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
    ...
    <div style={{ flex: 1, height: 18, backgroundColor: C.bgSection, borderRadius: RADIUS.sm, overflow: 'hidden' }}>
      <div
        style={{
          width: `${max > 0 ? Math.max(6, (count / max) * 100) : 0}%`,
          height: '100%',
    ...
    <div style={{ width: 26, textAlign: 'right', fontFamily: FONT.body, fontSize: 12, fontWeight: 700, color: C.textPrimary }}>
      {count}
    </div>
  </div>
);
      ...
      {data.map((d) => (
        <div key={d.date} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }} title={`${d.date}: ${d.count}`}>
          <div
            style={{
              width: '100%',
              height: `${(d.count / max) * 54}px`,
              minHeight: d.count > 0 ? 4 : 2,
              backgroundColor: d.count > 0 ? C.primary : C.border,
              borderRadius: 3,
            }}
          />
...
export const AuditInsights: React.FC<Props> = ({ type, id }) => {
  ...
  if (isError || !data) {
    return (
      <div style={{ textAlign: 'center', padding: '40px 0', color: C.danger, fontFamily: FONT.body, fontSize: 13 }}>
  ...
  if (data.totalRevisions === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '40px 0', color: C.textMuted, fontFamily: FONT.body, fontSize: 13 }}>
        <i className="bi bi-bar-chart" style={{ fontSize: 28, display: 'block', marginBottom: 8 }} aria-hidden />
  ...
  return (
    <div className="ci-fade-in" style={{ paddingTop: 4 }}>
      ...
      {/* Hot fields + Category mix */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14, marginBottom: 14 }}>
        <Panel title="Most-changed fields" icon="bi bi-fire">
          {data.hotFields.length === 0 ? (
            <div style={{ fontFamily: FONT.body, fontSize: 12, color: C.textMuted }}>No field changes.</div>
          ) : (
