...
const StatChip: React.FC<{ icon: string; label: string; n: number; color: string }> = ({
  icon,
  label,
  n,
  color,
...
const ModeBtn: React.FC<{ active: boolean; onClick: () => void; icon: string; label: string }> = ({
  active,
  onClick,
  icon,
  label,
...
const DiffRow: React.FC<{ change: V2FieldChange; mode: DiffMode }> = ({ change, mode }) => (
  ...
  >
    <div style={{ width: 150, flexShrink: 0 }}>
      <div style={{ fontFamily: FONT.body, fontSize: 13, fontWeight: 600, color: C.textPrimary }}>
        {change.fieldLabel}
      </div>
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      {mode === 'unified' ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <SideCell
            text={valueText(change.oldValue, change.oldValueFormatted)}
            tone={change.changeType === 'ADDED' ? 'empty' : 'old'}
          />
          <SideCell
            text={valueText(change.newValue, change.newValueFormatted)}
      ...
      {mode === 'sidebyside' && result.diffs.length > 0 && (
        <div style={{ display: 'flex', gap: 12, padding: '0 0 6px 162px' }}>
          <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <span style={{ fontFamily: FONT.body, fontSize: 11, fontWeight: 700, color: C.textMuted }}>
              R{result.from}
            </span>
            <span style={{ fontFamily: FONT.body, fontSize: 11, fontWeight: 700, color: C.textMuted }}>
              R{result.to}
