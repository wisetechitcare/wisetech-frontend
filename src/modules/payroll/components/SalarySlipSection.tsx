...
const SalarySlipSection: React.FC<SalarySlipSectionProps> = ({
    ...
    /** Below `sm` these two actions own the full row rather than hugging the right edge. */
    ...
    // On a phone these two actions share the row instead of being pushed against the right
    // edge: they were sized to their text, so a long label ("Generating…", "Email Slip")
    // left them crammed into whatever space was left over.
    const actionSizing: React.CSSProperties = isNarrow
        ? { flex: '1 1 0', minWidth: 0, padding: '10px 12px', fontSize: '13px' }   // 44px tall — a real touch target
        : { flex: '0 0 auto', padding: '7px 14px', fontSize: '12px' };
    ...
    const downloadBtnStyle: React.CSSProperties = {
        ...
        // Never wrap to two lines, but truncate rather than push the neighbour off-screen.
        whiteSpace: 'nowrap',
        overflow: 'hidden',
    ...
    return (
        ...
        >
            {salarySlipProps && (canFetchSlip || canExportSheet) ? (
                ...
                // The Dropdown renders a wrapper div around the toggle, so the flex sizing has
                // to land here too — otherwise the toggle stretches inside a wrapper that is
                // still only as wide as its text.
                <Dropdown align="end" style={isNarrow ? { flex: '1 1 0', minWidth: 0 } : undefined}>
                    <Dropdown.Toggle
                        as="button"
                        type="button"
                        id="salary-slip-download"
                        disabled={loading || isExporting !== null}
                        style={{ ...downloadBtnStyle, width: isNarrow ? '100%' : undefined }}
                        onMouseEnter={e => { if (!loading) { (e.currentTarget as HTMLButtonElement).style.background = '#f1f5f9'; (e.currentTarget as HTMLButtonElement).style.borderColor = '#cbd5e1'; } }}
                        onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = '#f8fafc'; (e.currentTarget as HTMLButtonElement).style.borderColor = '#e2e8f0'; }}
                    >
                        <KTIcon iconName="file-down" className="fs-6" />
